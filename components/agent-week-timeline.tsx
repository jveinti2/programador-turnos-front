...
function transformDaySchedule(
  daySchedule: AgentScheduleData["schedule"][string]
): TimelineBlock[] {
...
export function AgentWeekTimeline() {
  ...
  dayNames.forEach((dayName, dayIndex) => {
    ...
    const daySchedule = Object.keys(selectedAgent.schedule).find(
      (key) =>
        key
          .toLowerCase()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "") === normalizedDayName
    );
    ...
    if (daySchedule && selectedAgent.schedule[daySchedule]) {
      ...
      totalWeekWorkBlocks += workBlocks.reduce(
        (sum, b) => sum + (b.endBlock - b.startBlock),
        0
      );
      totalWeekBreakBlocks += breakBlocks.reduce(
        (sum, b) => sum + (b.endBlock - b.startBlock),
        0
      );
      totalWeekDiscBlocks += discBlocks.reduce(
        (sum, b) => sum + (b.endBlock - b.startBlock),
        0
      );
      daysWorked++;
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">
                {totalWeekWorkBlocks} bloques × 15min
              </div>
            </div>
            ...
            <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">
                {totalWeekBreakBlocks} bloques × 15min
              </div>
            </div>
            ...
            <div className="bg-red-50 dark:bg-red-950 p-3 rounded-lg">
              ...
              <div className="text-xs text-muted-foreground">
                {totalWeekDiscBlocks} bloques × 15min
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="relative bg-gray-50 dark:bg-gray-900 border rounded-lg p-4 overflow-x-auto max-w-full">
                    ...
                    <div className="relative flex-1 h-12 bg-white dark:bg-gray-800 rounded border">
                      {hasSchedule ? (
                        blocks.map((block) => (
                          <div
                            key={block.id}
                            className={`absolute h-10  text-white text-[10px] font-semibold flex items-center justify-center transition-colors cursor-pointer ${getBlockColor(
                              block.type
                            )}`}
                            style={{
                              left: `${(block.startBlock / 96) * 100}%`,
                              width: `${
                                ((block.endBlock - block.startBlock) / 96) * 100
                              }%`,
                              top: "4px",
                            }}
